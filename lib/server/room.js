...
room.create = function (type) {
	...
	db.set(id, { id: id, type: name, players: {} });

	if (Array.isArray(type) && type.length > 0) { room.create(type); }
};

room.get = function (id) { return db.get(id); };

room.all = function () {
	var result = {};
	db.forEach(function (key, val) { result[key] = val; });
	return result;
};

room.clear = function () { fs.unlink('rooms.dirty'); };
