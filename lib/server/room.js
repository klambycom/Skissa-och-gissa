...
room.roundsLeft = function (r) {
	return rooms[r].totalRounds - rooms[r].rounds;
};

room.available = function () {
	var someRoundsLeft = Object.keys(rooms).filter(function (r) { return room.roundsLeft(r) > 2; });

	return someRoundsLeft.map(function (r) {
		return {
			id: rooms[r].id,
			type: rooms[r].type,
			image: rooms[r].image,
			...
			nrOfPlayers: rooms[r].players.length,
...
room.nrOfSameType = function (r) {
	var type = rooms[r].type,
		find = function (i) { return rooms[i].type === type && Object.size(rooms[i].players) <= 7; };
