...
room.addPlayer = function (player, r) {
	...
	if (typeof data === 'undefined') {
		throw { name: 'GameNotFoundException', message: 'Can\'t add player since game is not found.' };
	} else if (data.players.length > 9) {
		throw { name: 'GameIsFullException', message: 'Can\'t add player since game is full.' };
	}
	// Add player to room
...
room.players = function (r) {
	if (typeof rooms[r] === 'undefined') {
		throw { name: 'GameNotFoundException', message: 'Can\'t get players since game is not found.' };
	}
