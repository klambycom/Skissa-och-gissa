...
game.getDataForClient = function (r) {
	return {
		id: games[r].id,
		type: games[r].type,
		image: games[r].image,
...
game.withMoreRounds = filter(function (r) { return game.roundsLeft(r) > 2; });

game.withEmptySpots = filter(game.notFull);

game.withMoreRoundsAndEmptySpots = game.withMoreRounds.composite(game.withEmptySpots);

//var filterNotDone = filter(function (r) { return games[r].totalRounds - games[r].rounds > 2; }),
//	filterNotFull = filter(function (r) { return Object.size(games[r].players) < games[r].maxPlayers; }),
//	allIds = Object.keys.curry(games),
//	available = filterNotDone.composite(filterNotFull, allIds).map.curry(game.getDataForClient);
//
//var games = { ... },
//
...
//
//var game = function (fn) { return function (g) { return fn(games[g]); }; },
...
game.available = function () {
	return game.withMoreRoundsAndEmptySpots(Object.keys(games)).map(game.getDataForClient);
};
...
game.nrOfSameType = function (type) {
	var find = function (i) { return games[i].type === type && Object.size(games[i].players) <= 7; };

	return Object.keys(games).filter(find).length;
};
